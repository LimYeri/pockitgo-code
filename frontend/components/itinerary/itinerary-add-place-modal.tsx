...
import { addItineraryPlace } from '@/lib/api';
import type { KakaoPlaceResult, Itinerary, PlaceCategory } from '@/types';

const PLACE_CATEGORIES: PlaceCategory[] = ['맛집', '카페', '관광지', '숙소', '액티비티'];

interface ItineraryAddPlaceModalProps {
  open: boolean;
  onClose: () => void;
  onAdded: (updatedDay: Itinerary) => void;
  roomId: string;
  date: string;
}

export function ItineraryAddPlaceModal({
  open,
  onClose,
  onAdded,
  roomId,
  date,
}: ItineraryAddPlaceModalProps) {
  ...
  const handleAdd = async () => {
    ...
    setIsAdding(true);
    try {
      const updatedDay = await addItineraryPlace(roomId, date, {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-md" showCloseButton={false}>
        <DialogHeader>
          <DialogTitle>장소 추가 ({date})</DialogTitle>
        </DialogHeader>
